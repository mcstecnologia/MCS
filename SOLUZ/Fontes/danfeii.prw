...
#DEFINE MAXITEMC   048  //38                                              // Máxima de caracteres por linha de produtos/serviços
...
For nZ := 1 To nLenDet
	...
	aAux := {}
	//AADD(aAux, AllTrim(TransForm(nQtd,TM(nQtd,TamSX3("D2_QUANT")[1],TamSX3("D2_QUANT")[2])))) TSC575 Alexandre Kuhnen - NYCOL - alterado
	//AADD(aAux, AllTrim(TransForm(nVUnit,TM(nVUnit,TamSX3("D2_PRCVEN")[1],TamSX3("D2_PRCVEN")[2])))) TSC575 - NYCOL - alterado
	AADD(aAux, AllTrim(TransForm(nQtd,"@E 999,999.99"))) 
	AADD(aAux, AllTrim(TransForm(nVUnit, "@E 999,999.9999")))
	...
	If Type("oNF:_INFNFE:_TOTAL:_ICMSTOT:_VBCST:TEXT") <> "U"
		If oNF:_INFNFE:_TOTAL:_ICMSTOT:_VBCST:TEXT <> "0"
			...
			If oNF:_INFNFE:_DEST:_ENDERDEST:_UF:TEXT == "SP"
				cAux += ": "
				aLote := RastroNFOr(SD2->D2_DOC,SD2->D21_SERIE,SD2->D2_CLIENTE,SD2->D2_LOJA)
...
nForTo		:=	Len(cStrAux)/31 //25
nForTo		+=	Iif(nForTo>Round(nForTo,0),Round(nForTo,0)+1-nForTo,nForTo)
For nX := 1 To nForTo
	oDanfe:Say(nLinCalc,098,SubStr(cStrAux,Iif(nX==1,1,((nX-1)*31)+1),31), oFont12N:oFont )
	nLinCalc+=10
Next nX
