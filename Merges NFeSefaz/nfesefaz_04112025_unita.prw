	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
			...
			Else
				...
				//GRUPO UNITA
				//SE HORA ESTIVER EM BRANCO PREENCHE PARA NÃO DAR ERRO DA TRANSMISSAO
				If alltrim(SF2->F2_HORA) == "" .and. cEmpAnt == "11"
					cTime := Time()
					RECLOCK( "SF2", .F. )
						SF2->F2_HORA := SubStr( cTime, 1, 2 )  + ":"  +SubStr( cTime, 4, 2 )
					MSUNLOCK()
				ENDIF
				...
				While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
					...
					If lContinua
						...
						If !AllTrim(SC5->C5_MENOTA2) $ cMensCli
							If Len(cMensCli) > 0 .And. SubStr(cMensCli, Len(cMensCli), 1) <> " "
								cMensCli += " "
							EndIf

							cMensCli += AllTrim(SC5->C5_MENOTA2)
						...
						*/
						...
						//CUSTOMIZADO LEPONO UNITA
						cRetForm := FORMULA(AllTrim(GetNewPar("MV_DECR35","")))
						if cRetForm <> nil .and. !AllTrim(cRetForm) $ cMensFis
							If Len(cMensFis) > 0 .And. SubStr(cMensFis, Len(cMensFis), 1) <> " "
								cMensFis += " "
							EndIf
				...
				*/
				...
				ElseIf !lEasy .Or. (lEasy .AND. (cAliasSD1)->D1_TIPO $ "NDCPI")
					...
					If MsSeek(xFilial("CD5")+(cAliasSD1)->D1_DOC+(cAliasSD1)->D1_SERIE+(cAliasSD1)->D1_FORNECE+(cAliasSD1)->D1_LOJA+(cAliasSD1)->D1_ITEM)
							aAdd(aDI,{;
								{"I04","NCM",CD5->CD5_XTEC},; 				//1 ---------------------------- aFill UNITA ----------------------------------- //
								{"I15","vFrete",0},;							//2
								{"I16","vSeg",0},;							//3
								...
								{"I29","vDescDI",0},;						//13
								{"N14","pRedBC",CD5->CD5_XRED },;			//14 ---------------------------- aFill UNITA ----------------------------------- //
								{"O11","qUnid",0},;							//15
								{"O12","vUnid",0},;							//16
...
/* Tratamento da data de saida da nota - GRUPO UNITA */
if cEmpAnt <> '11'		//apenas para empresa 11
	cString += NfeTag('<dhSaiEnt>',Iif(lDSaiEnt,"",ConvType(aNota[03])+"T"+Iif(Len(AllTrim(aNota[06])) > 5,ConvType(aNota[06]),ConvType(aNota[06])+":00")))
else
	dDtSai := aNota[03]
	if aNota[04] == '1'	//se nota de saida
		SF2->(dbSetOrder(1))
		if SF2->(MsSeek(xFilial("SF2")+aNota[02]+aNota[01],.T.))
			if (dtos(DataValida(dDtSai)) <> dtos(dDtSai)) .or. (val(substr(SF2->F2_ZHRFAT,1,2) + '.' + substr(SF2->F2_ZHRFAT,4,2)) > 13.00)
				dDtSai := DataValida(dDtSai + 1, .T.)
			Else
				dDtSai := DataValida(dDtSai + 1, .T.)
			endif
		endif
...
/*
// CUSTOMIZADO UNITA - CBENEF ////////
if len(aCredPresum) == 0
	cString += NfeTag('<cBenef>',ConvType(aProd[44]))

ElseIf Len(acredPresum) > 0 .and. (alltrim(aCST[2]) + alltrim(aCST[1])) == "120" 
	cString += NfeTag('<cBenef>',ConvType("SC820028"))

Endif

*/

/*Inicio Tratamento MCS*/

/*Baseado nessa condição abaixo
aadd(aCST,{IIF(!Empty((cAliasSD1)->D1_CLASFIS),SubStr((cAliasSD1)->D1_CLASFIS,2,2),'50'),;
IIF(!Empty((cAliasSD1)->D1_CLASFIS),SubStr((cAliasSD1)->D1_CLASFIS,1,1),'0')})

**Verificado primeiro o array aCst[2] + aCst[1], pois posição 2 é o CST e a Primeira é a origem
*/
If Substr((alltrim(aCST[2]) + alltrim(aCST[1])),1,2) == "20" 
	cString += NfeTag('<cBenef>',ConvType("SC820028"))
Endif
/*Fim tratamento MCS*/
