...
#DEFINE HMARGEM   030
#DEFINE VMARGEM   030
#DEFINE MAXITEM   010   //10/customizado Vedamotors                  // Mximo de produtos para a primeira pgina
...
#DEFINE MAXITEMC  052//030//012 //Vedamotors                         // Mxima de caracteres por linha de produtos/servios
#DEFINE MAXMENLIN 110                                                // Mximo de caracteres por linha de dados adicionais
...
#DEFINE MAXVALORC 012//008  //Vedamotors                             // Mximo de caracteres por linha de valores numricos
...
Private nAuxt10  := 5 //Vedamotors  
Private najuste := 86 //Vedamotors - utilizado para ajustar a coluna de descricao do produto

oDanfe:SetResolution(78) // Tamanho estipulado para a Da
oDanfe:SetLandscape()
...
private oNfe
Private _cNfiscal	:= "" // BOLETO
Private _cSerie		:= "" // BOLETO
Private _cCliente	:= "" // BOLETO
Private _cLoja		:= "" // BOLETO
Private _cPEDIDO	:= "" // BOLETO
Private _cBancoC5	:= "" // BOLETO
Private _cCondPgC5	:= "" // BOLETO

Default lIsLoja	:= .F.

aNFs:={}/// Vedamotors VED172
...
Else
	lContinua := Funname()=="VED172" .or. Funname()=="VED315" .or. Funname()=="VED456" .or. Pergunte("NFSIGW",.T.)  .AND. ( (!Empty(MV_PAR06) .AND. MV_PAR06 == 2) .OR. Empty(MV_PAR06) )   /// Vedamotors VED172
EndIf
	...
	If !lImpDir
		...
		While !Eof() .And. xFilial("SF3") == (cAliasSF3)->F3_FILIAL .And.;
			...
			If Len(aNotas) >= 50 .Or. 	(cAliasSF3)->(Eof())
				...
				aValid	:=U_fVerUser(aNotas,aXML) //// VedaMotors /// Funo para imprimir apenas notas geradas pelo usurio na rotina VED172
				aNotas	:=aValid[1]
				aXML	:=aValid[2]
						...
						If (!Empty(cAutoriza) .Or. !Empty(cCodAutDPEC) .Or. Alltrim(aXML[nX][8])$"2,5") .And. !cCodRetNFE $ RetCodDene()
							...
							Else
								...
								If MsSeek(xFilial("SF2")+aNotas[nX][05]+aNotas[nX][04]+aNotas[nX][06]+aNotas[nX][07]) .And. Alltrim(aXML[nX][8])$"1,3,4,6,7".Or. ( Alltrim(aXML[nX][8]) $ "2,5"  .And. !Empty(cAutoriza) )
									...
									If !SF2->F2_FIMP$"D"
										SF2->F2_FIMP := "S"
										aadd(aNFs,{SF2->F2_DOC,SF2->F2_SERIE})   //// VedaMotors VED172
									EndIf
							...
							If Empty(cAviso) .And. Empty(cErro)
								// Tratamento para impressao do boleto.
								_cNfiscal	:= aNotas[nX][05]
								_cSerie		:= aNotas[nX][04]
								_cCliente	:= aNotas[nX][06]
								_cLoja		:= aNotas[nX][07]
								_cPEDIDO	:= Posicione("SC6",4,xFilial("SC6")+_CNFISCAL+_CSERIE,"C6_NUM")
								_cBancoC5	:= Posicione("SC5",1,xFilial("SC5")+_CPEDIDO,"C5_BANCO")
								_cCondPgC5	:= Posicione("SC5",1,xFilial("SC5")+_CPEDIDO,"C5_CONDPAG")
		...
		While  !lBreak  .And. nLenarray >= 0
			...
			While !Eof() .And. !lBreak .And. ;
				...
				For nx :=1 To nLenNotas
					...
					MsSeek(cChaveSFT)				
					If ( !Empty(aXML[nX][2]) .AND. (AllTrim((cAliasSFT)->FT_ESPECIE)== 'SPED') .Or. (lImpDir .And. !Empty(aXML[nX][2])) ) .And. Empty((cAliasSFT)->FT_DTCANC) //Realizada tal alterao para que seja verificado antes da impresso se a NF-e est cancelada ou e do modelo sped
						If !Empty(aXml[nX])
							cAutoriza		:= aXML[nX][1]
							cCodAutDPEC	:= aXML[nX][5]
							cCodRetNFE		:= aXML[nX][9]
							...
							IF SF1->(!EOF()) .And. SF2->(FieldPos("F2_FILDEST"))> 0 .And. SF2->(FieldPos("F2_FORDES"))> 0 .And.SF2->(FieldPos("F2_LOJADES"))> 0 .And.SF2->(FieldPos("F2_FORMDES"))> 0 .And. !EMPTY (SF2->F2_FORDES)  
								...
								If SFT->(MsSeek(SF1->F1_FILIAL+"E"+SF1->F1_SERIE+SF1->F1_DOC+SF1->F1_FORNECE+SF1->F1_LOJA,.T.))
									Do While cChave == SFT->FT_FILIAL+"E"+SFT->FT_SERIE+SFT->FT_NFISCAL+SFT->FT_CLIEFOR+SFT->FT_LOJA .And. !SFT->(Eof())
								   	RecLock("SFT",.F.)  // thagor 16-08-2018 erro quando est para classificar automatico 
											SFT->FT_CHVNFE := SubStr(NfeIdSPED(aXML[nX][2],"Id"),4)
											MsUnLock()
											//Array criado para gravar o SF3 no final, pois a tabela SF3 pode estah em processamento quando se trata de DBF ou AS/400.
											If aScan(aGrvSF3,{|aX|aX[1]+aX[2]+aX[3]+aX[4]+aX[5]==(cAliasSFT)->(FT_SERIE+FT_NFISCAL+FT_CLIEFOR+FT_LOJA+FT_IDENTF3)})==0
...
U_fNfeImp(aNota[5],aNota[4]) // gravo que a nfe foi impressa mais uma vez.
...
Local nF3BaseIcm 	:= 0 // VEDAMOTORS
Local nF3ValIcm  	:= 0 // VEDAMOTORS
...
Local cUF           := ""
Local cChaveCont    := ""
Local cLogo         := FisxLogo("1")//"LogoDanfe.bmp"//FisxLogo("1") //VEDAMOTORS
...
//LMB
Local lUsaNGF := SuperGetMv("VD_NGF001", .F., .T.)
...
//nFaturas := IIf(oFatura<>Nil,IIf(ValType(oNF:_InfNfe:_Cobr:_FAT)=="A",Len(oNF:_InfNfe:_Cobr:_FAT),1),0)
	...
	For nX := 1 To 3
		...
		For nY := 1 To Min(9, nFaturas)
			Do Case
				Case nX == 1
					If nFaturas > 1
						AAdd(aAux, AllTrim(oFatura:_FAT[nY]:_nDup:TEXT))
					Else
						AAdd(aAux, AllTrim(oFatura:_FAT:TEXT))
					EndIf
				Case nX == 2
					If nFaturas > 1
						AAdd(aAux, AllTrim(ConvDate(oFatura:_FAT[nY]:_dVenc:TEXT)))
					Else
						AAdd(aAux, AllTrim(ConvDate(oFatura:_FAT[nY]:_dVenc:TEXT)))
					EndIf
				Case nX == 3
					If nFaturas > 1
						AAdd(aAux, AllTrim(TransForm(Val(oFatura:_FAT[nY]:_vDup:TEXT), "@E 9999,999,999.99")))
					Else
						AAdd(aAux, AllTrim(TransForm(Val(oFatura:_FAT[nY]:_vDup:TEXT), "@E 9999,999,999.99")))
					EndIf
			EndCase
...
For nZ := 1 To nLenDet
    ...
    Else
	   nVTotal  := Val(oDet[nX]:_Prod:_vProd:TEXT)//-Val(IIF(Type("oDet[nPrivate]:_Prod:_vDesc")=="U","",oDet[nX]:_Prod:_vDesc:TEXT))
	    //customizado vedamotors
	    if  ( aDest[07] == "EX"  .OR. ALLTRIM(SA1->A1_CONTA) == "1110010004" ) .AND. GETMV("VD_XIMPUM2")  == .T.
...
//	nPerDesc	:= ((nDesc*100)/nVTotal)
	...
	nMaxCod := MaxCod(oDet[nX]:_Prod:_cProd:TEXT, 80)//vedamotors
	
	// Tratamento para quebrar os digitos dos valores
	aAux := {}
	//AADD(aAux, AllTrim(TransForm(SubStr(cValToChar(nQtd),1,at(cValToChar(nQtd),',')+3),TM(nQtd,TamSX3("D2_QUANT")[1],TamSX3("D2_QUANT")[2]))))
	AADD(aAux, AllTrim(TransForm(nQtd,TM(nQtd,TamSX3("D2_QUANT")[1],3))))
   ...
   If ( oDet[nX]:_Prod:_uTrib:TEXT <> oDet[nX]:_Prod:_uCom:TEXT )  .and. (  aDest[07] == "EX" .OR. ALLTRIM(SA1->A1_CONTA) == "1110010004" )  .AND. GETMV("VD_XIMPUM2")  == .T.
	...
	While !Empty(cAux) .Or. !Empty(cAuxItem) .Or. !Empty(aAux[1]) .Or. !Empty(aAux[2]) .Or. !Empty(aAux[3]) .Or. !Empty(aAux[4]);
	       ...
	       .Or. !Empty(aAux[11])
		nMaxCod := MaxCod(cAuxItem, 80)//vedamotors
...
If Type("oIdent:_tpAmb:TEXT")<>"U" .And. oIdent:_tpAmb:TEXT=="2"
	cAux := "DANFE emitida no ambiente de homologaçãoo - SEM VALOR FISCAL"
	While !Empty(cAux)
...
//31/03  Especfico Vedamotors
//Tamanho Das Colunas, Realizado Ajuste
aTamCol[1] := 61//aTamCol[1]  
aTamCol[2] := 137+najuste//aTamCol[2] +8//-10//+11
aTamCol[3] := 34//aTamCol[3] -5//- 3//NCM  
aTamCol[4] := 17//aTamCol[4] -2//- 3//NCM   
aTamCol[6] := 15//aTamCol[6] - 8//UN   
aTamCol[5] := 21//aTamCol[5] - 2//UN 
aTamCol[7] := 33//aTamCol[8] - 3//UN
aTamCol[8] := 46//aTamCol[8] - 3//UN 
aTamCol[9] := 46//aTamCol[10] - 3//UN
aTamCol[10] := 37//aTamCol[10] - 3//UN 
aTamCol[11] := 41//aTamCol[10] - 3//UN
aTamCol[12] := 45//aTamCol[10] - 3//UN
aTamCol[13] := 35//aTamCol[10] - 3//UN
aTamCol[14] := 46//aTamCol[10] - 3//UN
aTamCol[15] := 40//aTamCol[10] - 3//UN
aTamCol[16] := 47//aTamCol[16] -4//- 4	   
aTamCol[17] := 42//aTamCol[17] //+1	  
aTamCol[18] := 32//aTamCol[18] - 2   	 
aTamCol[19] := 32//aTamCol[19] - 2 
//Fim Vedamotors
...
For nY := 1 To nLenItens
	...
	Else
		...
		oDanfe:Say(nLinha, aColProd[6][1] + 2, aAux[1][6][nY], oFont07:oFont)
			nAuxH2 := aColProd[7][1] + ((aColProd[7][2] - aColProd[7][1] - 2) - RetTamTex(aAux[1][7][nY], oFont07:oFont, oDanfe)) -1//Vedamotors
		oDanfe:Say(nLinha, nAuxH2, aAux[1][7][nY], oFont07:oFont)
		
		nAuxH2 := aColProd[8][1] + ((aColProd[8][2] - aColProd[8][1] - 2) - RetTamTex(aAux[1][8][nY], oFont07:oFont, oDanfe)) -1//Vedamotors
		oDanfe:Say(nLinha, nAuxH2, aAux[1][8][nY], oFont07:oFont)
		
		nAuxH2 := aColProd[9][1] + ((aColProd[9][2] - aColProd[9][1] - 2) - RetTamTex(aAux[1][9][nY], oFont07:oFont, oDanfe)) -1//Vedamotors
		oDanfe:Say(nLinha, nAuxH2, aAux[1][9][nY], oFont07:oFont)
	//Especfico Vedamotors
		//nAuxH2 := aColProd[10][1] + ((aColProd[10][2] - aColProd[10][1] - 2) - RetTamTex(aAux[1][10][nY],amdin oFont07:oFont, oDanfe)) + 2
	  ...
	  /*	nAuxH2 := aColProd[11-1][1] + ((aColProd[11-1][2] - aColProd[11-1][1] - 2) - RetTamTex(aAux[1][11][nY], ofont07:oFont, oDanfe))  //vuniliq
		nAuxH2 := nAuxH2 - nauxT10//Vedamotors
		//nAuxH2 := nAuxH2 - 5 //Vedamotors
		oDanfe:Say(nLinha, nAuxH2, aAux[1][11][nY], ofont07:oFont)
		
		nAuxH2 := aColProd[12-1][1] + ((aColProd[12-1][2] - aColProd[12-1][1] - 2) - RetTamTex(aAux[1][12][nY], ofont07:oFont, oDanfe)) + 7 //Total Liq.
		nAuxH2 := nAuxH2 - nauxT10//Vedamotors
		...
		*/              
		//Fim Vedamotors
		nAuxH2 := aColProd[13-3][1] + ((aColProd[13-3][2] - aColProd[13-3][1] - 2) - RetTamTex(aAux[1][13][nY], ofont07:oFont, oDanfe)) + 2//8  //bicms
		nAuxH2 := nAuxH2 - nauxT10//Vedamotors
		oDanfe:Say(nLinha, nAuxH2, aAux[1][13][nY], ofont07:oFont)
		
		nAuxH2 := aColProd[14-3][1] + ((aColProd[14-3][2] - aColProd[14-3][1] - 2) - RetTamTex(aAux[1][14][nY], ofont07:oFont, oDanfe)) + 2   //bicms st
		nAuxH2 := nAuxH2 - nauxT10//Vedamotors
		//nAuxH2 := nAuxH2 - 5 //Vedamotors

		oDanfe:Say(nLinha, nAuxH2, aAux[1][14][nY], ofont07:oFont)
		
		nAuxH2 := aColProd[15-3][1] + ((aColProd[15-3][2] - aColProd[15-3][1] - 2) - RetTamTex(aAux[1][15][nY], ofont07:oFont, oDanfe)) + 2  //vl icm
	   	//nAuxH2 := nAuxH2 - 5 //Vedamotors
		nAuxH2 := nAuxH2 - nauxT10//Vedamotors

		oDanfe:Say(nLinha, nAuxH2, aAux[1][15][nY], ofont07:oFont)
		
		nAuxH2 := aColProd[16-3][1] + ((aColProd[16-3][2] - aColProd[16-3][1] - 2) - RetTamTex(aAux[1][16][nY], ofont07:oFont, oDanfe)) + 2   //vlripi
	   nAuxH2 := nAuxH2 - nauxT10//Vedamotors
	   //	nAuxH2 := nAuxH2 - 5 //Vedamotors

		oDanfe:Say(nLinha, nAuxH2, aAux[1][16][nY], ofont07:oFont)
		
		nAuxH2 := aColProd[17-3][1] + ((aColProd[17-3][2] - aColProd[17-3][1] - 2) - RetTamTex(aAux[1][17][nY], ofont07:oFont, oDanfe)) + 2	  //icms
		//nAuxH2 := nAuxH2 - 5 //Vedamotors
		nAuxH2 := nAuxH2 - nauxT10//Vedamotors

		oDanfe:Say(nLinha, nAuxH2, aAux[1][17][nY], ofont07:oFont)
		
		nAuxH2 := aColProd[18-3][1] + ((aColProd[18-3][2] - aColProd[18-3][1] - 2) - RetTamTex(aAux[1][18][nY], ofont07:oFont, oDanfe)) + 1//5 //ipi
	    nAuxH2 := nAuxH2 - nauxT10//Vedamotors
		//nAuxH2 := nAuxH2 - 5//Vedamotors

		oDanfe:Say(nLinha, nAuxH2, aAux[1][18][nY], ofont07:oFont)
		nAuxH2 := aColProd[19-3][1] + ((aColProd[19-3][2] - aColProd[19-3][1] - 2) - RetTamTex(aAux[1][19][nY], ofont07:oFont, oDanfe)) -2//+ 4 Vedamotors
...
// IMPRESSAO DO BOLETO.
If getmv("VED_IMPBOL") .AND. _cCondPgC5 <> "001"

	If Funname()=="VED172"
       SF2->(DBSetorder(1))
       SF2->(DBSeek(xFilial("SF2")+_CNFISCAL+_CSERIE))
    Endif
	// verificar se cliente tem credito.
	...
	BeginSql Alias cAliasSE1
		...
		((SE1.E1_TIPO = 'NCC') OR (SE1.E1_TIPO = 'RA ' )) AND
		SE1.E1_SALDO > 0 AND
		SE1.%NotDel%
	EndSql

	/*cPara   := "financeiro3@vedamotors.com.br"
	cMsg	:= "Cliente : "+SF2->F2_CLIENTE+"-"+SF2->F2_LOJA+" com RA ou NCC em aberto"*/

	If (cAliasSE1)->QTIT > 0
		//Alert('Cliente com RA ou NCC em aberto....')

		//u_VDEnvMail(cMsg,'Faturamento NFE : '+SF2->F2_DOC,cPara,,,"DANFE")
		
	Else
		
		If(Funname()<>"VED456")

				If(SUPERGETMV("DANFE3BOLE", .T.,xFilial("SF2")))
					If _cBancoC5 == "001"
						//LMB
						If lUsaNGF
							cMsgAguarde := "Gerando borderôs B.Brasil para o Novo Gestor Financeiro..."
						Else
							cMsgAguarde := "Imprimindo Boleto B.Brasil..."
						EndIf
						MsAguarde({|| U_bol005a(_CSERIE,_CNFISCAL,_CNFISCAL,5,"",1,"",.f.,oDanfe)}, cMsgAguarde)
					EndIF
					if _cBancoC5 == "237"
						MsAguarde({|| U_bol005BA(_CSERIE,_CNFISCAL,_CNFISCAL,5,"",1,"",.f.,oDanfe)},"Imprimindo Boleto Bradesco...")
					EndIF
					if _cBancoC5 == "341"
						//LMB
						If lUsaNGF
							cMsgAguarde := "Gerando borderôs Itau para o Novo Gestor Financeiro..."
						Else
							cMsgAguarde := "Imprimindo Boleto Itau..."
						EndIf
						MsAguarde({|| U_bol005IA(_CSERIE,_CNFISCAL,_CNFISCAL,5,"",1,"",.f.,oDanfe)},cMsgAguarde)
					EndIF
					
					/* Implementado para desbloquear registros lockados pelos programas BOL005*  */
					DBCOMMITALL()
...
Local cLogoD	:= "LogoDanfe.bmp"//"" // VEDAMOTORS
...
If nFolha ==1
	...
	IF SF2->F2_TIPO == 'N' // Especifico Vedamotors - Solicitado em 20/04/2016 - Thiago SLA
		oDanfe:Say(nLin,nBaseTxt,"RESSALVAMOS QUE OS PRAZOS PARA RECLAMAÇÕES SÃO:",oFont07N:oFont)
		nLin:= nLin+10
		oDanfe:Say(nLin,nBaseTxt,"DIVERGÊNCIAS OU TROCAS DE ITENS: 72 HORAS",oFont07N:oFont)
		nLin:= nLin+10
		oDanfe:Say(nLin,nBaseTxt,"DIVERGÊNCIAS QUANTO AO NÚMERO DE VOLUMES FAZER RESSALVA NO CONHECIMENTO DE TRANSPORTE NO ATO DO RECEBIMENTO",oFont07N:oFont)
		nLin:= nLin+10
	ENDIF
...
Static Function RetTamTex(cTexto, oFont, oPrinter)
	...
	nTamanho := nTamanho*0.65//80 vedamotors
