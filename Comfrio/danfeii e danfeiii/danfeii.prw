...
Private lAuto	    := Iif(isInCallStack("U_DIATMS04") .OR. (IsInCallStack("U_UNFE") .AND. (IsInCallStack("U_CFAFAT72") .OR. isInCallStack("U_DEFIEDAT"))),.T.,.F.)//Personalizado - Incluído por Carlos Niemeyer - FSW - TOTVS RP - 04/09/2014
...
If !lAuto //Personalizado - Incluído por Carlos Niemeyer - FSW - TOTVS RP - 04/09/2014
...
If lExistNFe
	...
	/*
		Personalizado - Carlos Niemeyer - 22/08/2014
	*/
	//--Início   /*
	If checaImpressaoBoleto() //Aqui ja foram passados os parametros para o boleto
		showLog("[DANFEII] Impressão do Boleto Ativada.")
		If !lAuto
			MsgRun("Imprimindo Boletos...","TOTVS",{|| imprimeBoleto(oSetup,oDanfe) })
		Else
			MsgRun("Imprimindo Boletos...","TOTVS",{|| imprimeBoleto(Nil,oDanfe,oClassUNFe:getPrinter()) })
		Endif
	Else
		showLog("[DANFEII] Impressão do Boleto Desativada.")
	Endif
	//--Término
Else
	If !lIsLoja
...
local lChave	:= .F.
Local lImprime	 := .F. //Personalizado Carlos Niemeyer - FSW - TOTVS RP - 10/09/2014

Default lIsLoja	:= .F.

//Personalizado Carlos Niemeyer - FSW - TOTVS RP - 10/09/2014
//--Início
If lAuto
	lImprime := .T.
Else
	lImprime := Pergunte("NFSIGW",.T.)
Endif
//--Término

//If Pergunte("NFSIGW",.T.) //Personalizado Carlos Niemeyer - FSW - TOTVS RP - 10/09/2014
If lImprime
...
If Type("oNF:_InfNfe:_infAdic:_infCpl:TEXT")<>"U"
	If at("<", oNF:_InfNfe:_infAdic:_InfCpl:TEXT) <> 0
		...
		cAux := stripTags(oNF:_InfNfe:_infAdic:_InfCpl:TEXT, .T.)
	endIf
	//Personalizado - Incluído por Matheus Dasuke - FSW - TOTVS RP - 28/08/2014
	//Tratamento para Quebrar as Mensagens do XML que estiverem com o marcador em várias linhas
	//--Inicio
	If !Empty(cAux) .And. ("#" $ cAux) //Se o marcador for encontrado
		cAux2 := ""
		For nCont:=1 to Len(cAux)  //Percorre o texto em busca do marcador
			If SubStr(cAux,nCont,1) == "#" .Or. nCont == Len(cAux) //Se encontrado ou estiver no fim da mensagem processa a quebra com o texto encontrado antes do marcador
				//Se for o último caracter da mensagem adiciona na mensagem
				If nCont == Len(cAux)
				...
				EndDo
				cAux2 := ""
			Else //caso contrário somente adiciona o novo texto na var. auxiliar
				cAux2 += SubStr(cAux,nCont,1)
			Endif
		Next nCont
	Else
		...
		If Len(oWs:oWSRETORNANOTASNXRESULT:OWSNOTAS:OWSNFES5) > 0
			...
			/*For nX := 1 To Len(aIdNfe)
				If aScan(aResposta,{|x| x[4] == aIdNfe[nX,04] .And. x[5] == aIdNfe[nX,05] })==0
					...
					u_cfconout("Falta")
					u_cfconout(aIdNfe[nX][4]+" - "+aIdNfe[nX][5])
...
/*
Função Personalizada para Impressão do Boleto
*/
Static Function imprimeBoleto(oSetup,oDanfe,cImpressoraInf)	
	Local aParam 			:= {}
	Local cCodDef			:= GetNewPar("ZZ_IMPBOL","005")
	Local cTipoDef			:= "I"
	Local lShowPerg			:= .F.
	Local lAuto				:= .F.
	Local lImprimeBoleto	:= .T.
	Local lRet				:= .T.
	Local nMvPar01			:= val(MV_PAR01)
	Local nMvPar02			:= val(MV_PAR02)
	Local nMvpar03          := Val(MV_PAR01)
	...
	Private cFiltrosBoleto	:= "" //getFiltrosBoleto()
	Private cPrinterDANFE	:= ""
	Default cImpressoraInf	:= ""

	Private DANFE_MV_PAR01	:= MV_PAR01
	Private DANFE_MV_PAR02	:= MV_PAR02
	Private DANFE_MV_PAR03	:= MV_PAR03

	If ValType(oSetup) == "O"
		If oSetup:GetProperty(PD_PRINTTYPE) == IMP_SPOOL
			//If MsgYesNo("Deseja Imprimir o Boleto na Mesma Impressora Selecionada da DANFE? (Se marcar NÃO irá imprimir nas impressoras parametrizadas da impressão automática).","TOTVS")
			cPrinterDANFE := Alltrim(oDanfe:cPrinter)
		Else
			/*
			Não imprimirá se selecionado a Impressora de PDF
			*/
			//lImprimeBoleto := .F.
			cPrinterDANFE := AllTrim(GetNewPar("ZZ_PRTPDF","PDFCreator"))
			If !( MsgYesNo("Deseja Imprimir o Boleto Bancário na Impressora '" + cPrinterDANFE + "'? .","TOTVS") )
				lImprimeBoleto := .T.//26112016
			Endif
		Endif
	Else
		cPrinterDANFE := AllTrim(cImpressoraInf)
	Endif

	IF lImprimeBoleto //.And. !Empty(cCodDef)
		showLog("[DANFEII][BOLETO] Impressora Selecionada: " + cPrinterDANFE)

		For nMvpar01:= nMvpar01 To nMvpar02 //Localiza a conta do cliente conforme NF
			nC := nC + 1				
			DBSelectArea("SF2") 
			SF2->(DBSetOrder(1))
			SF2->(DBGotop())
			IF (SF2->(DBSeek(xFilial("SF2")+STRZERO(nMvpar01,9)+cSerie)))//NOTA + SERIE
				DBSelectArea("SF3") 
				SF3->(DBSetOrder(1))
				SF3->(DBGotop())

				IF(SF3->(DBSeek(xFilial("SF3")+DTOS(SF2->F2_EMISSAO)+SF2->F2_DOC+SF2->F2_SERIE+SF2->F2_CLIENTE+SF2->F2_LOJA)))

					IF EMPTY(SF3->F3_DTCANC) .and. !EMPTY(SF3->F3_CHVNFE)		    					
						AADD(aCliente, SF2->F2_CLIENTE)						
						AADD(aLoja, SF2->F2_LOJA)
						//SF2->(DBCloseArea())

						//Rotina para Retornar o Banco/Agencia/Conta e Sub-Conta associada a uma Rede ou Cliente informado					
						aConta := u_CFAFATB8(SF2->F2_CLIENTE,SF2->F2_LOJA,'',IF(SF2->(FieldPos("F2_XSUPAPR"))>0,SF2->F2_XSUPAPR,' '))
						nNewRecno:= ExecBlock("CFAFATLU",.F.,.F.,{SF2->F2_DOC,SF2->F2_SERIE})

						If !Empty(SE1->E1_PORTADO) .and. SE1->E1_PORTADO<>aConta[1]
							aConta[1] := SE1->E1_PORTADO
							aConta[2] := SE1->E1_AGEDEP
							aConta[3] := SE1->E1_CONTA
						EndIf


						IF aConta[1] == '246'// BANCO ABC	
							nNewRecno:= ExecBlock("CFAFATLU",.F.,.F.,{SF2->F2_DOC,SF2->F2_SERIE})
							IF 	nNewRecno > 0
								Aadd(aABCArray,{ .T., nNewRecno,aConta[1],aConta[2],aConta[3]})					
							ENDIF	
						ElseIf aConta[1] == '237' .AND. SF2->(FieldPos("F2_XSUPAPR")) > 0 .and.  !EMPTY(SF2->F2_XSUPAPR) .AND. SF2->F2_XSUPAPR != 'N'// BANCO Bradesco						 
							nNewRecno:= ExecBlock("CFAFATLU",.F.,.F.,{SF2->F2_DOC,SF2->F2_SERIE})
							IF 	nNewRecno > 0
								Aadd(aBRDSup,{ .T., nNewRecno,aConta[1],aConta[2],aConta[3]})					
							EndIf	

						ElseIf aConta[1] == '341' .AND. SF2->(FieldPos("F2_XSUPAPR")) > 0 .and.  !EMPTY(SF2->F2_XSUPAPR) .AND. SF2->F2_XSUPAPR != 'N'// BANCO ITAU						 
							nNewRecno:= ExecBlock("CFAFATLU",.F.,.F.,{SF2->F2_DOC,SF2->F2_SERIE})
							IF 	nNewRecno > 0
								Aadd(aItaSup,{ .T., nNewRecno,aConta[1],aConta[2],aConta[3]})					
							EndIf
					
						ELSEIF aConta[1] == '033'// BANCO SANTANDER						 
							nNewRecno:= ExecBlock("CFAFATLU",.F.,.F.,{SF2->F2_DOC,SF2->F2_SERIE})
							IF 	nNewRecno > 0
								Aadd(aSANArray,{ .T., nNewRecno,aConta[1],aConta[2],aConta[3]})					
							ENDIF	

						ELSEIF aConta[1] == '341' // BANCO ITAU						 
							nNewRecno:= ExecBlock("CFAFATLU",.F.,.F.,{SF2->F2_DOC,SF2->F2_SERIE})
							IF 	nNewRecno > 0
								Aadd(aITAArray,{ .T., nNewRecno,aConta[1],aConta[2],aConta[3]})					
							ENDIF
						ELSEIF aConta[1] == '107' // BANCO BBM						 
							nNewRecno:= ExecBlock("CFAFATLU",.F.,.F.,{SF2->F2_DOC,SF2->F2_SERIE})
							IF 	nNewRecno > 0
								Aadd(aBBMArray,{ .T., nNewRecno,aConta[1],aConta[2],aConta[3]})					
							ENDIF
						ELSEIF aConta[1] == '237' // BANCO Bradesco - SIFRA						 
							nNewRecno:= ExecBlock("CFAFATLU",.F.,.F.,{SF2->F2_DOC,SF2->F2_SERIE})
							IF 	nNewRecno > 0
								Aadd(aBRDArray,{ .T., nNewRecno,aConta[1],aConta[2],aConta[3]})					
							ENDIF
						ELSEIF aConta[1] == '274' // BANCO GRAFENO						 
							nNewRecno:= ExecBlock("CFAFATLU",.F.,.F.,{SF2->F2_DOC,SF2->F2_SERIE})
							IF 	nNewRecno > 0
								Aadd(aGRAFArray,{ .T., nNewRecno,aConta[1],aConta[2],aConta[3]})					
							ENDIF
						ELSEIF aConta[1] == '637' // BANCO ITAU - SOFISA						 
							nNewRecno:= ExecBlock("CFAFATLU",.F.,.F.,{SF2->F2_DOC,SF2->F2_SERIE})
							IF 	nNewRecno > 0
								Aadd(aSOFArray,{ .T., nNewRecno,aConta[1],aConta[2],aConta[3]})					
							ENDIF				
							/*ELSE
							cFiltroA += "'"+STRZERO(nMvpar01,9) +"',"*/						    	                     
						//ENDIF -- SANTANA --> ERRO NO MOMENTO DE COMPILAR... ENDIF COMENTADO   				
							/*ELSE
							cFiltroA += "'"+STRZERO(nMvpar01,9) +"',"*/						    	                     
						ELSEIF aConta[1] == '707' // BANCO DAYCOVAL
							nNewRecno:= ExecBlock("CFAFATLU",.F.,.F.,{SF2->F2_DOC,SF2->F2_SERIE})
							IF 	nNewRecno > 0
								Aadd(aDAYArray, nNewRecno)
							ENDIF
						ENDIF                                        
		...
		//IMPRIMI BOLETOS ABC
		IF Len(aABCArray) > 0
			u_IaBolABC(aABCArray) 
		ENDIF

		//IMPRIMI BOLETOS SANTANDER
		IF Len(aSANArray) > 0
			u_IaBolSAN(aSANArray) 
		ENDIF

		//IMPRIMI BOLETOS ITAU
		IF Len(aITAArray) > 0
			u_IaBolITA(aITAArray) 
		ENDIF

		//IMPRIMI BOLETOS BBM
		IF Len(aBBMArray) > 0
			u_IaBolBRD(aBBMArray) 
		ENDIF
		
		//IMPRIMI BOLETOS Bradesco - SIFRA
		IF Len(aBRDArray) > 0
			u_IaBolBRS(aBRDArray) 
		ENDIF
	
		//IMPRIME BOLETOS GRAFENO
		IF Len(aGRAFArray) > 0
			u_IaBolGRAF(aGRAFArray,,,oSetup)
		ENDIF

		//IMPRIMI BOLETOS Bradesco - Supplier
		IF Len(aBRDSup) > 0
			u_SupBolet(aBRDSup) 
		ENDIF
		...
		//IMPRIMI BOLETOS ITAU - SOFISA
		IF Len(aSOFArray) > 0
			u_IaBolSOF(aSOFArray) 
		ENDIF
		
		//IMPRIMI BOLETOS DAYCOVAL
		IF Len(aDAYArray) > 0
			U_DIAFIN10(aDAYArray , .T. , .F.)
		ENDIF
	
		/*
		IF  !EMPTY(cFiltroA)
		cFiltroA		:= Substr(cFiltroA,1,len(cFiltroA)-1)
		cFiltrosBoleto	:= getFiltroBoletoB(cFiltroA)
		aParam 			:= {lAuto,cTipoDef,cCodDef,lShowPerg}//"ZZ_IMPBOL","005"	
		lRet 			:= u_RunIEDat(aParam,.T.)
		ENDIF
		*/

		//Recarrega o Grupo de Perguntas da DANFE
		Pergunte("NFSIGW",.F.)
	ENDIF
Return (lRet)

Static Function getFiltroBoletoB(lFiltroA) 

	Local cFiltroB := ""		    
	Local lConverteAspas := .T.

	cFiltroB := " AND E1_NUM IN (" + lFiltroA + " ) " 
	cFiltroB += " AND E1_SERIE = '" + MV_PAR03 + "' "	
	/*
	Filtra Somente Clientes que geram boleto
	*/	
	dbSelectArea("SA1")	
	If SA1->(FieldPos("A1_ZZBOL")) > 0
		cFiltroB  += " AND A1_ZZBOL = 'S' "

	Endif

	If lConverteAspas
		cFiltroB := StrTran(cFiltroB,"'",Chr(34))
	Endif
Return (cFiltroB)	  

/*
Retorna os Filtros para Impressão do Boleto
*/

Static Function getFiltrosBoleto(lConverteAspas)
	Local cFiltrosBoleto := ""   

	Default lConverteAspas := .T.

	cFiltrosBoleto := " AND E1_NUM BETWEEN '" + MV_PAR01 + "' AND '" + MV_PAR02 + "' "
	cFiltrosBoleto += " AND E1_SERIE = '" + MV_PAR03 + "' "	
	...
	*/
	...
	dbSelectArea("SA1")	
	If SA1->(FieldPos("A1_ZZBOL")) > 0
		cFiltrosBoleto  += " AND A1_ZZBOL = 'S' "
	Endif

	If lConverteAspas
		cFiltrosBoleto := StrTran(cFiltrosBoleto,"'",Chr(34))
	Endif

Return (cFiltrosBoleto)

/*
Função que verifica se Imprime o Boleto
*/
Static Function checaImpressaoBoleto()
	Local cSQL				:= ""
	Local cAlias			:= ""               
	Local lConverteAspas	:= .F.
	Local cFiltrosBoleto  	:= getFiltrosBoleto(lConverteAspas)
	//Local cFiltroBoletoB	:= getFiltroBoletoB(lConverteAspas)
	Local lRet				:= .F.

	cSQL := " SELECT " + CRLF
	cSQL += " 	Count(*) AS CONTATITULOS " + CRLF
	cSQL += " FROM " + CRLF	
	cSQL += " 	" + RetSqlTab("SE1") + CRLF 
	cSQL += " 	INNER JOIN " + RetSqlTab("SA1") + " ON 1=1 " + CRLF 	
	cSQL += " 				AND " + RetSqlFil("SA1") + CRLF
	cSQL += " 				AND SA1.A1_COD = SE1.E1_CLIENTE " + CRLF
	cSQL += " 				AND SA1.A1_LOJA = SE1.E1_LOJA " + CRLF
	cSQL += " 				AND " + RetSqlDel("SA1") + CRLF	
	cSQL += " WHERE " + CRLF
	cSQL += " 	1=1 " + CRLF
	cSQL += " 	AND " + RetSqlFil("SE1") + CRLF
	cSQL += " 	AND SE1.E1_SALDO > 0 " + CRLF	
	cSQL += " 	" + cFiltrosBoleto//cFiltroBoletoB
	cSQL += " 	AND " + RetSqlDel("SE1") + CRLF
	...
	dbSelectArea(cAlias)
	If (cAlias)->(!EOF())
		showLog("[DANFEII] Títulos a serem Impressos Boletos: " + cValToChar( (cAlias)->CONTATITULOS ) )
		If (cAlias)->CONTATITULOS > 0
			lRet := .T.
		Else 
			lRet := .F.
		Endif
...
*/
Static Function showLog(cMensagem,lAtivaLog,cPrefixo)	
	Default cMensagem 	:= ""
	Default lAtivaLog	:= GetNewPar("ZZ_LOGS",.T.)
	Default cPrefixo	:= "[DANFEII]"//[" + DtoC(DDATABASE) + "][" + Time() + "] "	
	
	If lAtivaLog
		U_CFCONOUT(cPrefixo + OEMToAnsi(cMensagem))
	Endif
Return 
