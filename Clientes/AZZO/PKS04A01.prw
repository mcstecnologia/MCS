...
User Function PKS4A1GT(cAlias,nReg,nOpc)
	...
	Local cOpcao 		:= ""
	Local cConteudo 	:= ""
	Local cEmail		:= "preproduto@azzobrasil.com.br"
	Local cCampos 		:= "B1_TIPO,B1_COD,B1_DESC,B1_GRUPO,B1_UM,B1_LOCPAD,B1_MSBLQL,B1_CODBAR"
	...
	CriaCampo(@oDlg,nLin,nCol+160,@cCodBar,aCampos[8],,lCond)
...
Static Function PKSVALID()
	...
	Local lRet := .T.
	Local aVer := {cTipo,cCod,cDesc,cUm,cLocPad,cMSBLQL,cGrupo}
	Local nX := 0
...
Static Function CriaCampo(oParent,nLin, nCol, uConteudo, cTitulo, _cPict, _lBlock, _bValid, _cF3)
	...
	If  DbSeek(cTitulo)
		...
		/*If cTitulo == "B1_CODBAR"
			cVldSis := GetSX3Cache(cTitulo, "X3_VALID")
			bValid := &("{|| "+cVldSis + "}")*/
		//EndIf
	...
	Else
		...
		Else 
			@ nLin+nDif,nCol MSGET oCampo VAR B1_CODBAR SIZE nWidth,nHeight OF oParent PIXEL COLOR CLR_BLUE When IIF(_lBlock <> Nil, _lBlock, .T.) Valid Eval(bValid) Picture cPict F3 cF3 HasButton
		EndIf
	EndIf
